...
def get_location_data_for_id(id):
	id = str(id) if isinstance(id, int) else id
	result = query_db('SELECT * FROM CrimeLocation WHERE location_data = ' + id)
	if len(result) == 0:
		return {}
...
def get_all_data_for_id(id):
	ret_dict = get_crime_data_for_id(id)
	ret_dict.update(get_location_data_for_id(ret_dict['location_id']))
	return ret_dict
